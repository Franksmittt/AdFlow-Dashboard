...
const TaskEditorModal = ({ isOpen, onClose, onSave, taskToEdit, campaigns }) => {
    ...
    useEffect(() => {
        if (taskToEdit) {
            ...
            setCampaign(taskToEdit.campaign || (campaigns[0]?.name || ''));
...
export default function TasksPage() {
    ...
    const { tasks, campaigns, saveData, deleteData, loading } = useAppContext();
    ...
    // UPDATED: handleSaveTask now uses the generic saveData function
                            ...
                            <div key={status} onDragOver={handleDragOver} onDrop={(e) => handleDrop(e, status)} className="bg-gray-900/70 border-dashed border-2 border-transparent data-[is-over=true]:border-yellow-400 rounded-xl flex flex-col min-w-[300px]" data-is-over={draggedTask && draggedTask.status !== status}>
                                ...
                                <div className="p-4 space-y-4 overflow-y-auto flex-1">
                                    {tasks.filter(t => t.status === status).map(task => (
                                        <TaskCard key={task.id} task={task} onEdit={handleOpenEditModal} onDelete={handleDelete} onDragStart={(e) => handleDragStart(e, task)} isDragging={draggedTask && draggedTask.id === task.id} />
                                    ))}
                                </div>
                            </div>
                        ))}
